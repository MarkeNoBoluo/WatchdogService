...
DWORD WINAPI ServiceWorkerThread(LPVOID lpParam) {
    ...
    while (WaitForSingleObject(g_ServiceStopEvent, 5000) != WAIT_OBJECT_0) {
        iteration++;
        //LOG_DEBUG(L"看门狗服务运行中... 迭代次数: %d", iteration);

        // 📌 TODO: 这里将添加监控其他服务状态的逻辑
        // 示例：每10次迭代记录一次INFO日志
        if (iteration % 10 == 0) {
            //LOG_INFO(L"服务持续运行中，已迭代 %d 次", iteration);
        }
    }
...
VOID WINAPI ServiceMain(DWORD argc, LPTSTR* argv) {
    ...
    if (!hThread) {
        //LOG_ERROR(L"创建工作线程失败! 错误: %d\n", GetLastError());
        ReportServiceStatus(SERVICE_STOPPED, GetLastError(), 0);
        return;
    }

    // 步骤6: 等待停止事件（阻塞直到收到停止信号）
    //LOG_INFO(L"服务主线程等待停止事件...\n");
...
int _tmain(int argc, TCHAR* argv[]) {
    ...
    if (argc > 1) {
        if (_tcsicmp(argv[1], _T("install")) == 0) {
            ...
            // 检查管理员权限（建议但不是必须）
            if (!IsUserAnAdmin()) {
                //LOG_ERROR(L"警告：建议以管理员身份运行安装。\n");
            }

            if (InstallService()) {
                //LOG_INFO(L"\n安装成功！\n");
                //LOG_INFO(L"接下来可以:\n");
                //LOG_INFO(L"1. 在服务管理器中启动服务\n");
                //LOG_INFO(L"2. 使用命令行: sc start WatchdogService\n");
                //LOG_INFO(L"3. 重启计算机，服务将自动启动\n");
            }
            else {
                //LOG_ERROR(L"\n安装失败！\n");
            }
            ...
            //LOG_INFO(L"调试模式 - 以控制台程序运行\n");
            //LOG_INFO(L"=== 看门狗服务调试模式 ===\n");
            //LOG_INFO(L"按Ctrl+C停止程序\n\n");
            ...
            //LOG_INFO(L"未知参数: %s\n", argv[1]);
            //LOG_INFO(L"可用参数:\n");
            //LOG_INFO(L"  install     - 安装服务\n");
            //LOG_INFO(L"  uninstall   - 卸载服务\n");
            //LOG_INFO(L"  status      - 查看服务状态\n");
            //LOG_INFO(L"  debug       - 调试模式运行\n");
            //LOG_INFO(L"  run         - 直接运行（测试）\n");
            return 1;
        }
    }
    ...
    */
    if (!StartServiceCtrlDispatcher(ServiceTable)) {
        ...
        //LOG_ERROR(L"StartServiceCtrlDispatcher失败! 错误: %d\n", error);

        // 常见错误：不是从SCM启动（例如直接双击运行）
        if (error == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT) {
            //LOG_ERROR(L"错误：请以服务方式启动本程序\n");
            //LOG_ERROR(L"使用方法:\n");
            //LOG_ERROR(L"  安装服务: WatchdogService.exe install\n");
            //LOG_ERROR(L"  调试运行: WatchdogService.exe debug\n");
            //LOG_ERROR(L"  卸载服务: WatchdogService.exe uninstall\n");
        }
